...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($reservation) {
            ...
            if ($checkIn === '' || $checkOut === '') {
                ...
                if (!$result['success']) {
                    ...
                    header('Location: current_guests.php?updated=1');
                    exit;
                }
            }
        }
    ...
    if ($action === 'update_status') {
        ...
        $reservation = getReservationById($reservationId);
        $roomId = $reservation ? (int) $reservation['room_id'] : 0;
...
<body class="page-guest-stays">
    ...
    <script>
        ...
        document.getElementById('editGuestForm').addEventListener('submit', async function(e) {
            ...
            try {
                const res = await fetch('current_guests.php', {
                    method: 'POST',
                    body: formData,
                ...
                if (data.success && data.updated_guest) {
                    ...
                    if (tr) {
                        ...
                        tr.cells[1].textContent = updated.guest_name;
                        tr.cells[2].textContent = updated.room_number;
                        tr.cells[3].textContent = formatDate(updated.check_in_date);
                        tr.cells[4].textContent = formatDate(updated.check_out_date);
